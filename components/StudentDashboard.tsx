...
const StudentDashboard: React.FC<Props> = ({ user, activeTab, setActiveTab }) => {
  ...
  const renderSectionHeader = (title: string, desc: string, icon: any, colorClass: string, score?: number) => (
      <div className={`${colorClass} rounded-[56px] p-16 text-white shadow-2xl relative overflow-hidden mb-16`}>
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-10">
              <div className="max-w-3xl">
                  <div className="bg-white/10 p-4 rounded-3xl w-fit mb-8 backdrop-blur-xl border border-white/20 shadow-lg">
                    {React.createElement(icon, { className: "w-10 h-10" })}
                  </div>
                  <h2 className="text-6xl font-black tracking-tighter leading-none">{title}</h2>
                  <p className="mt-8 text-white/90 font-medium text-2xl leading-relaxed italic">{desc}</p>
              </div>
              <div className="shrink-0 flex gap-6">
                  <div className="bg-white/10 backdrop-blur-xl px-12 py-10 rounded-[40px] border border-white/20 text-center min-w-[200px] shadow-2xl">
                      <p className="text-[11px] font-black uppercase tracking-[0.3em] opacity-60 mb-2">Readiness Index</p>
                      <p className="text-6xl font-black">{score || 0}<span className="text-2xl ml-1 opacity-50">%</span></p>
                  </div>
              </div>
          </div>
          <div className="absolute -top-20 -right-20 w-[600px] h-[600px] bg-white/5 rounded-full blur-[120px] pointer-events-none"></div>
          <div className="absolute -bottom-40 -left-40 w-[400px] h-[400px] bg-black/5 rounded-full blur-[80px] pointer-events-none"></div>
      </div>
  );
  ...
  const renderDashboard = () => (
    <div className="space-y-16 animate-fadeIn pb-24 font-inter">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-10">
            <div className="flex-1">
                <h1 className="text-7xl font-black text-slate-900 tracking-tighter leading-none">Welcome, {user.name.split(' ')[0]}</h1>
                <div className="mt-8 flex items-start gap-6 max-w-2xl">
                    <Quote className="w-10 h-10 text-primary-600/20 shrink-0" />
                    <p className="text-slate-400 font-medium text-2xl tracking-tight leading-relaxed italic">
        ...
        <section className="space-y-10 relative">
            <div className="flex items-center justify-between relative z-10">
                <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-4xl font-black text-slate-900 tracking-tighter">Your Next Milestone</h2>
                </div>
            </div>
            
            <div className="relative z-10">
                {data.readinessExam && (
                    <div className="animate-flow rounded-[72px] p-16 shadow-2xl transition-all group overflow-hidden border border-primary-100/50">
                        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-primary-500/5 rounded-full blur-[100px] -mr-40 -mt-40 pointer-events-none"></div>
                        <div className="relative z-10 flex flex-col lg:flex-row gap-20">
                            <div className="flex-1 flex flex-col">
                                <div className="flex justify-between items-start mb-10">
                                    <div className="bg-white p-5 rounded-[28px] text-primary-600 shadow-xl border border-primary-50 group-hover:scale-110 group-hover:rotate-3 transition-all duration-500">
                                        <Zap className="w-10 h-10" fill="currentColor" />
                                    </div>
                                    <span className="px-6 py-2.5 bg-primary-600 text-white text-[11px] font-black uppercase rounded-2xl shadow-lg shadow-primary-600/20 tracking-[0.3em]">Upcoming Exam</span>
                                </div>
                                <h3 className="text-6xl font-black text-slate-900 mb-8 tracking-tighter leading-[0.95]">
                                    {data.readinessExam.title}
                                </h3>
                                <p className="text-slate-500 text-xl font-medium mb-16 leading-relaxed max-w-xl italic">
                                    {data.readinessExam.description}
                                </p>
                                
                                <div className="mt-auto flex flex-wrap gap-6 mb-12">
                                    <div className="flex items-center gap-3 px-8 py-4 bg-white rounded-[24px] border border-primary-100 shadow-sm text-slate-500 font-black text-xs uppercase tracking-widest">
                                        <Clock className="w-5 h-5 text-primary-500" /> {data.readinessExam.durationMins}m Duration
                                    </div>
                                    <div className="flex items-center gap-3 px-8 py-4 bg-white rounded-[24px] border border-primary-100 shadow-sm text-slate-500 font-black text-xs uppercase tracking-widest">
                                        <ShieldCheck className="w-5 h-5 text-green-500" /> Verified Assessment
                                    </div>
                                </div>

                                <button 
                                    onClick={() => setActiveExam(data.readinessExam)}
                                    className="w-full lg:w-fit px-20 bg-primary-600 text-white font-black py-8 rounded-[40px] shadow-2xl shadow-primary-600/30 hover:bg-slate-900 transition-all duration-500 flex items-center justify-center text-sm uppercase tracking-[0.4em] active:scale-95 group/btn"
                                >
                                    Initiate Exam <ArrowRight className="ml-5 w-6 h-6 group-hover/btn:translate-x-3 transition-transform" />
                                </button>
                            </div>

                            <div className="lg:w-[400px] flex flex-col gap-8">
                                <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.5em] mb-4">Preparation Schedule</h4>
                                ...
                                <div className="space-y-6">
                                    {[
                                        ...
                                        <div key={i} className="p-8 bg-white rounded-[40px] border border-primary-50 flex items-center gap-8 group/slot hover:shadow-xl transition-all duration-300">
                                            <div className="w-16 h-16 rounded-[24px] bg-primary-50 flex flex-col items-center justify-center shrink-0 border border-primary-100">
                                                <span className="text-[9px] font-black text-primary-400 uppercase leading-none mb-1">{slot.slot}</span>
                                                <span className="text-xl font-black text-primary-600 leading-none">{slot.day.split(' ')[1]}</span>
                                            </div>
                                            <div className="flex-1">
                                                <p className="text-lg font-black text-slate-900 tracking-tight">{slot.title}</p>
                                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2">{slot.day} • {slot.time}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
        ...
        <div className="grid xl:grid-cols-12 gap-12">
            <div className="xl:col-span-8 space-y-12">
                <div className="bg-white p-16 rounded-[64px] shadow-sm border border-slate-100 relative overflow-hidden group">
                    <div className="mb-12 relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                        <div>
                           <div className="flex items-center gap-4 mb-2">
                               <div className="p-3 bg-primary-50 rounded-2xl text-primary-600">
                                   <BarChart3 className="w-6 h-6" />
                               </div>
                               <h3 className="font-black text-slate-900 text-3xl tracking-tighter">Skill Analysis Spectrum</h3>
                           </div>
                           <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.4em] ml-1">Comprehensive readiness breakdown</p>
                        </div>
                        <div className="flex gap-4">
                            <div className="flex items-center gap-2 px-6 py-3 bg-slate-50 rounded-2xl border border-slate-100">
                                <div className="w-3 h-3 rounded-full bg-primary-600"></div>
                                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Current Score</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col lg:flex-row items-center gap-16 relative z-10">
                        ...
                        <div className="flex-1 w-full space-y-6">
                            <h4 className="text-[10px] font-black text-slate-300 uppercase tracking-[0.4em] mb-4">Mastery Indicators</h4>
                            <div className="grid grid-cols-1 gap-4">
                                {data.analytics.map((item: any, i: number) => (
                                    <div key={i} className="flex items-center justify-between p-6 bg-slate-50 rounded-3xl border border-slate-100 hover:bg-white hover:border-primary-100 hover:shadow-xl transition-all duration-300 group/item">
                                        <div className="flex items-center gap-4">
                                            <div className="w-2 h-8 bg-slate-200 rounded-full group-hover/item:bg-primary-600 transition-colors"></div>
                                            <div>
                                                <p className="text-xs font-black text-slate-900 uppercase tracking-tight">{item.subject}</p>
                                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Mastery Level</p>
                                            </div>
                                        </div>
                                        <div className="text-right">
                                            <span className="text-xl font-black text-slate-900 tracking-tighter">{item.A}%</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
            ...
            <div className="xl:col-span-4 space-y-12">
                <div className="bg-white p-16 rounded-[64px] border border-slate-100 shadow-sm flex flex-col items-center text-center h-full justify-center group overflow-hidden relative">
                    <div className="absolute inset-0 bg-primary-50 opacity-0 group-hover:opacity-100 transition-opacity duration-1000"></div>
                    <div className="bg-slate-950 p-10 rounded-[48px] text-white mb-10 shadow-2xl group-hover:scale-110 group-hover:rotate-6 group-hover:bg-primary-600 transition-all duration-700 relative z-10">
                        <UserCheck className="w-20 h-20" />
                    </div>
                    <div className="relative z-10">
                        <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.5em] mb-4">Behavioral Profile</p>
                        <h3 className="text-5xl font-black text-slate-900 mb-6 tracking-tighter leading-none">Career Ready</h3>
                        <p className="text-slate-400 font-medium italic text-lg leading-relaxed max-w-xs mx-auto opacity-80">"Highly aligned with modern workplace values and professional expectations."</p>
                    </div>
                    <div className="mt-12 w-full p-8 bg-slate-50 rounded-[40px] border border-slate-100 relative z-10 group-hover:bg-white transition-colors">
                       <div className="flex justify-between items-center mb-4">
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Alignment Score</span>
                          <span className="text-xl font-black text-primary-600">96%</span>
                       </div>
